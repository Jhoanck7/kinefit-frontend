...
import { BackendService, BackendSpecialist } from "@/types";
...
export default function EspecialistasPage() {
  ...
  const [especialistaAEliminar, setEspecialistaAEliminar] =
    useState<BackendSpecialist | null>(null);
  ...
  const [especialistaAEditarId, setEspecialistaAEditarId] = useState<
    number | null
  >(null);
  const [editFormData, setEditFormData] = useState<BackendSpecialist | null>(
    null
  );
  ...
  async function handleCrearEspecialista(e: React.FormEvent) {
    ...
    try {
      const sId =
        nuevoServicioId === "0" || nuevoServicioId === 0
          ? undefined
      ...
      const msg =
        err instanceof Error
          ? err.message
          : "No se pudo crear el integrante del equipo.";
      alert(msg);
    } finally {
      setCreando(false);
    }
  ...
  async function handleGuardarEspecialista(
    e: React.FormEvent,
    especialista: BackendSpecialist
  ) {
    e.preventDefault();
    setGuardandoId(especialista.id);
    try {
      const sId = especialista.servicio?.id
        ? especialista.servicio.id
        : undefined;

      const actualizada = await specialistService.updateEspecialista(
        especialista.id,
        {
      ...
      setEspecialistas(prev =>
        prev.map(item =>
          item.id === especialista.id ? { ...item, ...actualizada } : item
        )
      );
      setNotificacion(
        `Los datos de ${especialista.nombre} fueron guardados correctamente.`
      );
      ...
      alert(
        "No se pudo actualizar la información del integrante en el servidor."
      );
    } finally {
      setGuardandoId(null);
    }
  ...
  async function ConfirmarEliminacion() {
    ...
    try {
      ...
      setEspecialistas(prev =>
        prev.filter(e => e.id !== especialistaAEliminar.id)
      );
      setNotificacion(
          ...
          {especialistas.map(esp => (
            <div
              ...
              onClick={() => {
                ...
                setEditFormData({
                  ...esp,
                  servicio: esp.servicio ? { ...esp.servicio } : null,
                });
              }}
            >
              ...
              <div className="p-2 sm:p-4">
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-200">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        setEspecialistaAEliminar(
                          especialistas.find(orig => orig.id === esp.id) || esp
                        );
                        setEspecialistaAEditarId(null);
                        setEditFormData(null);
                      }}
                ...
                <div className="pt-2 bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <ImageUploader
                    ...
                    value={esp.fotoUrl || ""}
                    onChange={secureUrl =>
                      setEditFormData({ ...esp, fotoUrl: secureUrl })
                    }
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <TextField
                      ...
                      value={esp.nombre}
                      onChange={e =>
                        setEditFormData({ ...esp, nombre: e.target.value })
                      }
                      obligatorio
                      required
                    />
                    ...
                    <TextField
                      ...
                      value={esp.cargo}
                      onChange={e =>
                        setEditFormData({ ...esp, cargo: e.target.value })
                      }
                      obligatorio
                      required
                    />
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={e => {
                          ...
                          if (val === "0") {
                            ...
                            const sFound = servicios.find(
                              s => s.id === Number(val)
                            );
                            setEditFormData({
                              ...esp,
                              servicio: sFound
                                ? { id: sFound.id, nombre: sFound.nombre }
                                : null,
                            });
                          }
                        }}
                      ...
                      >
                        <option value="0">
                          Sin Servicio Clínico (Gerencia / Dirección /
                          Administración)
                        </option>
                        {servicios.map(srv => (
                    ...
                    <TextField
                      ...
                      value={esp.email || ""}
                      onChange={e =>
                        setEditFormData({ ...esp, email: e.target.value })
                      }
                    />
          ...
          <div className="p-6 space-y-6 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600 leading-relaxed">
                ...
                <span className="font-bold text-slate-900">
                  {especialistaAEliminar.nombre}
                </span>{" "}
                ({especialistaAEliminar.cargo}) del sistema.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 p-3.5 rounded-2xl text-left">
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-xs text-brand-primary font-medium truncate">
                  {especialistaAEliminar.cargo}
                </p>
                <p className="text-[11px] text-slate-500 truncate">
                  {especialistaAEliminar.email || "Sin correo"}
                </p>
              </div>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-left">
              <p className="text-xs text-amber-800 leading-normal">
                <span className="font-bold">⚠️ Nota Importante:</span> Esta
                acción eliminará su registro de la base de datos. Si el
                especialista posee citas asociadas, te recomendamos cancelar el
                borrado y simplemente{" "}
