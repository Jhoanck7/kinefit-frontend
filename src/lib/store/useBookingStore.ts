...
export const useBookingStore = create<BookingState>((set, get) => ({
    ...
    try {
      ...
      const msg =
        err instanceof Error
          ? err.message
          : "Error al conectar con la API de servicios.";
      set({
        services: [],
        isLoading: false,
  ...
  fetchSpecialists: async (servicioId: number) => {
    ...
    try {
      const response = await appointmentService.getEspecialistas(
        servicioId,
        true
      );
      set({
      ...
      const msg =
        err instanceof Error
          ? err.message
          : "Error al consultar especialistas.";
      set({
        specialists: [],
        isLoading: false,
        backendConnected: false,
  ...
  fetchAvailableSlots: async (especialistaId: number, date: string) => {
    ...
    try {
      const response = await appointmentService.getBloques(
        especialistaId,
        date
      );
      set({
      ...
      const msg =
        err instanceof Error
          ? err.message
          : "Error al consultar bloques de horarios.";
      set({
        availableSlots: [],
        isLoading: false,
        backendConnected: false,
  ...
  authenticateWithGoogle: async (idToken: string) => {
    ...
    try {
      ...
      set({
        authToken: token,
        patientName:
          `${res.data.paciente.nombre} ${res.data.paciente.apellido}`.trim(),
  ...
  submitBookingAndPay: async () => {
    ...
    try {
      ...
      if (!tokenToUse) {
        throw new Error(
          "Debes iniciar sesión con Google para agendar tu reserva."
        );
      }
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Error al procesar la reserva y pago en el backend.";
      set({
        error: errorMessage,
        isSubmitting: false,
      });
