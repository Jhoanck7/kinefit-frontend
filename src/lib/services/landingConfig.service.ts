...
export const landingConfigService = {
  async getConfig(): Promise<LandingConfigData> {
    try {
      const response = await apiClient.get<{ data: LandingConfigData }>(
  ...
  async updateConfig(
    ...
    const response = await apiClient.put<{ data: LandingConfigData }>(
      "/configuracion/landing",
      dto,
      headers
    );
  ...
  async patchConfig(
    partialDto: Partial<LandingConfigData>,
    token?: string
  ): Promise<LandingConfigData> {
    const headers: Record<string, string> = {};
    if (token) {
    ...
    const response = await apiClient.patch<{ data: LandingConfigData }>(
      "/configuracion/landing",
      partialDto,
      headers
    );
  ...
  async sincronizarGoogleReviews(
    token?: string
  ): Promise<{ data: LandingConfigData; message: string }> {
    const headers: Record<string, string> = {};
    if (token) {
      headers["Authorization"] = `Bearer ${token}`;
    }
    const response = await apiClient.post<{
      ...
      message: string;
    }>("/configuracion/sincronizar-google-reviews", {}, headers);
    return response;
  },
};
