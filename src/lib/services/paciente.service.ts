...
export const pacienteService = {
  async getAll(busqueda?: string, soloActivos?: boolean): Promise<PacienteBackendDto[]> {
    const params = new URLSearchParams();
    if (busqueda) params.append('busqueda', busqueda);
    if (soloActivos !== undefined) params.append('soloActivos', String(soloActivos));
    const query = params.toString();
    const res = await apiClient.get<unknown>(`/pacientes${query ? `?${query}` : ''}`);
    const obj = (res as { data?: unknown })?.data ?? res;
    if (Array.isArray(obj)) return obj as PacienteBackendDto[];
    if (obj && typeof obj === 'object' && 'items' in obj && Array.isArray((obj as { items: unknown }).items)) {
  ...
  async getById(id: number | string): Promise<PacienteBackendDto> {
    ...
    return ((res as { data?: PacienteBackendDto })?.data ?? res) as PacienteBackendDto;
  },

  async verificarRut(rut: string): Promise<{ existe: boolean; paciente?: PacienteBackendDto }> {
    const res = await apiClient.get<unknown>(
      `/pacientes/verificar-rut?rut=${encodeURIComponent(rut)}`
    );
    return ((res as { data?: { existe: boolean; paciente?: PacienteBackendDto } })?.data ?? res) as {
      existe: boolean;
  ...
  async update(id: number | string, dto: UpdatePacienteDto): Promise<PacienteBackendDto> {
