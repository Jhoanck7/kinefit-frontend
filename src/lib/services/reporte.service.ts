...
export interface ReporteVentasResponseDto {
  ...
  ventasPorMetodo: Array<{ metodoPago: string; monto: number; cantidad: number }>;
  movimientos: Array<{
    id: number;
    fecha: string;
    monto: number;
...
export const reporteService = {
  async getReporteVentas(filtros?: {
    ...
    return apiClient.get<ReporteVentasResponseDto>(`/reportes/ventas${query ? `?${query}` : ''}`);
  },
  ...
  async getReporteReservas(filtros?: {
    ...
    if (filtros?.fechaDesde) params.append('fechaDesde', filtros.fechaDesde);
    if (filtros?.fechaHasta) params.append('fechaHasta', filtros.fechaHasta);
    if (filtros?.compararCon !== undefined) params.append('compararCon', String(filtros.compararCon));
    if (filtros?.vista) params.append('vista', filtros.vista);

    const query = params.toString();
    return apiClient.get<ReporteReservasResponseDto>(`/reportes/reservas${query ? `?${query}` : ''}`);
  },
  ...
  async getReporteComisiones(filtros?: {
    ...
    const query = params.toString();
    return apiClient.get<ReporteComisionesResponseDto>(`/reportes/comisiones${query ? `?${query}` : ''}`);
  },
};
