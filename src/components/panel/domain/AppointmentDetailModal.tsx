...
export function AppointmentDetailModal({
  ...
  return (
    <Modal abierto={Boolean(citaId)} onCerrar={alCerrar} ancho="max-w-3xl">
      {!cita ? (
        <div className="p-8 text-center font-sans text-xs text-slate-500">Cargando reserva…</div>
      ) : (
        <DetalleCita
          cita={cita}
          guardando={guardando}
    ...
    <div className="bg-white text-slate-900 font-sans shadow-none rounded-none">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h2 className="font-sans text-sm font-bold uppercase tracking-wider text-slate-900">
              Detalle de Reserva <span className="font-sans font-bold text-slate-900">#{cita.id}</span>
            </h2>
          ...
          <p className="font-sans text-xs text-slate-500 mt-0.5">
            Creada el <span className="font-sans text-slate-700 font-medium">{formatearFechaHora(cita.creadaEn)}</span>
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="md:col-span-2 p-6 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <div>
                <span className="font-sans text-[11px] font-medium text-slate-400 uppercase tracking-wider block">Especialista</span>
              ...
              <div>
                <span className="font-sans text-[11px] font-medium text-slate-400 uppercase tracking-wider block">Fecha y Horario</span>
                <span className="font-sans font-medium text-sm text-slate-900 block mt-0.5">
              ...
              <div>
                <span className="font-sans text-[11px] font-medium text-slate-400 uppercase tracking-wider block">Estado Actual</span>
                <div className="flex items-center gap-1.5 mt-1">
                  <span className={`h-2 w-2 rounded-full ${dotColor}`} aria-hidden />
                  <span className={`font-sans text-xs font-bold uppercase tracking-wider ${textoColor}`}>
                    {definicion.etiqueta}
                  </span>
                </div>
              </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                <span className="font-sans text-[11px] font-medium text-slate-400 uppercase tracking-wider block">Monto Anticipo</span>
              ...
              {cita.webpayTransaccionId && (
                <div>
                  <span className="font-sans text-[11px] font-medium text-slate-400 uppercase tracking-wider block">N° Transacción Webpay</span>
                  <span className="font-sans font-medium text-sm text-slate-900 mt-0.5 block">{cita.webpayTransaccionId}</span>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-2">
                {cita.notas.paciente && (
                  <div className="border border-slate-200 bg-slate-50/60 p-3 rounded-none">
                    <span className="font-sans text-[10px] uppercase font-medium text-slate-400 block mb-1">Nota para Paciente</span>
                    <span className="font-sans font-medium text-sm text-slate-900">{cita.notas.paciente}</span>
                  </div>
                )}
                {cita.notas.interna && (
                  <div className="border border-slate-200 bg-slate-50 p-3 rounded-none">
                    <span className="font-sans text-[10px] uppercase font-medium text-slate-400 block mb-1">Nota Interna del Personal</span>
                    <span className="font-sans font-medium text-sm text-slate-900">{cita.notas.interna}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="md:col-span-1 bg-slate-50/80 p-6 flex flex-col justify-between space-y-6">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="font-sans font-medium text-sm text-slate-900 mt-0.5 truncate" title={cita.paciente.correo}>
      ...
      <div className="border-t border-slate-200 bg-slate-50/60 p-4">
        {definicion.acciones.length === 0 ? (
          <p className="font-sans text-xs text-slate-500 text-center">{definicion.explicacionSinAcciones}</p>
        ) : (
          <div className="flex flex-wrap gap-2 justify-end">
            {definicion.acciones.map((accion) => {
              ...
              let estiloBtn = "border border-slate-200 bg-white hover:bg-slate-50 text-slate-900 shadow-none";
              if (esPrimario) {
                estiloBtn = "bg-[#003366] text-white hover:bg-[#002244] border-0 font-bold shadow-none";
              } else if (esPeligro) {
                estiloBtn = "border border-red-300 bg-red-50 text-red-700 hover:bg-red-100 shadow-none";
              }
