    ...
    <CollapsibleSection titulo="Historial de la cita" abiertaPorDefecto={false}>
      <ol className="space-y-3">
        {historial.map((cambio, indice) => (
          <li key={indice} className="flex items-start justify-between gap-3 text-sm">
            <div>
              ...
              {cambio.motivo && <p className="text-xs text-brand-muted italic">{cambio.motivo}</p>}
            </div>
            <span className="shrink-0 text-xs text-brand-muted">{formatearFechaHora(cambio.fecha)}</span>
          </li>
        ))}
      </ol>
    </CollapsibleSection>
