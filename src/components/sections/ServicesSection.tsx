...
export default function ServicesSection({
  ...
  const serviceIcons = [
    ...
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 009 11v-1.5M12 11c0-3.517 1.009-6.799 2.753-9.571m-3.44 2.04l-.054-.09A13.916 13.916 0 0015 11v1.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
      />
    </svg>,
  ];
  ...
  return (
    <section
      id="services"
      className="py-24 bg-white border-b border-slate-200/60 text-slate-900"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          <p className="text-slate-500 mt-4 text-base sm:text-lg">
            Combinamos evidencia clínica, terapeutas certificados y equipamiento
            moderno para ofrecerte una recuperación efectiva.
          </p>
        </div>
          ...
          {displayServices.map((service, index) => (
            ...
            >
              <div>
                ...
                {/* Title */}
                <h3 className="text-xl font-bold leading-8 text-slate-900 mb-4">
                  {service.nombre}
                </h3>
                ...
                {service.features && service.features.length > 0 && (
                  <ul className="space-y-3 mb-8 border-t border-slate-200/60 pt-6">
                    {service.features.map(feat => (
                      <li
                        key={feat}
                        className="flex items-start gap-2.5 text-xs font-medium text-brand-muted"
                      >
                        <svg
