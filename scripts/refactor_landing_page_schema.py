...
PAGE_PATH = "src/app/(panel)/panel/(shell)/landing/page.tsx"

with open(PAGE_PATH, "r", encoding="utf-8") as f:
    content = f.read()

# We need to completely rewrite the rendering logic in page.tsx while keeping state management for arrays.
# It's actually easier to just replace the whole file since we are doing a massive architectural rewrite.
...
export default function ConfigLandingPage() {
  ...
  function handleAgregarProcessStep() {
    setProcessSteps((prev) => [...prev, { id: prev.length + 1, title: "Nuevo Paso", description: "", iconName: "star" }]);
  }
  ...
  async function handleGuardar(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await landingConfigService.saveConfig(dataToSave);
      setFormData(res);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 pb-12">
                      ...
                      {processSteps.map((step, idx) => (
                        <div key={idx} className="rounded-xl border border-brand-border bg-white p-4 space-y-3">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <TextField etiqueta="Nombre Icono (opcional)" value={step.iconName || ""} onChange={(e) => handleProcessStepChange(idx, "iconName", e.target.value)} />
                          </div>
                          <div>
      ...
      <SimulatedActionNotice
        abierto={guardadoExitoso}
        onCerrar={() => setGuardadoExitoso(false)}
        titulo="Configuración Actualizada"
        descripcion="Los nuevos textos de todas las secciones han sido guardados en el servidor backend."
      />
    </div>
  );
}
...
print("Successfully generated and replaced page.tsx")
